...
test('browser login accepts only provider OAuth URLs and cancellation waits for native process exit',async()=>{
 ...
 for(const [provider,url]of [['Codex','https://auth.openai.com/oauth/authorize?state=x'],['Claude','https://claude.ai/oauth/authorize?state=x'],['Grok','https://auth.x.ai/oauth2/authorize?state=x']]){
