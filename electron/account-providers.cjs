...
const providers=Object.freeze({
 ...
 Claude:{provider:'Claude',executable:'claude.exe',model:'opus',write:true,installUrl:'https://code.claude.com/docs/en/setup',loginArgs:p=>['auth','login','--claudeai',...(p.email?['--email',p.email]:[])],authHosts:['claude.ai','claude.com'],authPath:/^\/(?:cai\/)?oauth\/authorize$/},
...
function providerCatalog(){return Object.values(providers).map(({provider,executable,model,write,installUrl})=>{let installed=false,resolved='';try{resolved=require('./discovery.cjs').resolveExecutable({provider,executable});installed=true;}catch{}return {provider,executable:resolved||executable,model,write,installUrl,installed};});}
