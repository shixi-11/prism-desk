...
function pluginSources(cacheRoot, errors) {
  ...
  const add = (root, marketplace) => {
    ...
    try {
      ...
      candidates.push({ id, kind: 'plugin', name: manifest.name, displayName, marketplace, version, path: real(root), roots, dependencies: !!(manifest.apps || manifest.mcpServers || manifest.mcp || fs.existsSync(path.join(root, '.mcp.json')) || fs.existsSync(path.join(root, '.app.json'))) });
    } catch { errors.push({ name: path.basename(root), reason: '插件清单无法识别，未接入。' }); }
  };
  if (fs.existsSync(path.join(cacheRoot, '.codex-plugin', 'plugin.json'))) add(cacheRoot, 'selected');
...
class SharedCapabilities {
  constructor({ config, directory, home = '', pluginCaches } = {}) {
    this.config = config;
  ...
  collect(state = this.state()) {
    ...
    for (const cache of caches.filter(exists)) for (const plugin of pluginSources(cache, errors)) {
    ...
    const plugins = [...pluginMap.values()].map(plugin => {
      ...
      const selected = state.plugins.includes(plugin.id);
      if (selected) entries.push(...found);
      return { ...plugin, selected, count: found.length, entries: found };
    });
  ...
  view({ check = false } = {}) {
    ...
    return {
      ...
      sources: current.sources, plugins: current.plugins.map(({ roots, entries, ...plugin }) => plugin),
      changes, errors: current.errors, assistant: current.assistant, assistantChanged,
  ...
  sync(token, pluginId) {
    ...
    if (pluginId !== undefined) {
      if (!view.plugins.some(p => p.id === pluginId && p.count > 0)) throw Error('插件来源不可用，请重新检查。');
  ...
  removeSource(id, token) {
    ...
    else if (state.plugins.includes(id)) state.plugins = state.plugins.filter(p => p !== id);
...
function manager() {
  if (!instance) {
    ...
    instance = new SharedCapabilities({ config: CONFIG, directory: process.env.PRISM_TEST_DATA || path.join(root, '.local'), home: process.env.PRISM_TEST_DATA ? '' : process.env.USERPROFILE || require('node:os').homedir() });
  }
  return instance;
}
