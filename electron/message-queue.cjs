...
class MessageQueue extends EventEmitter{
 ...
 async send(id){
  ...
  try{
   if((!item.options?.planning||this.runner.active?.planning)&&(!task.planReviewRequired||this.runner.active?.planning)&&await this.runner.steer(item.taskId,item.text,item.images)){
   ...
   const queued=!!this.runner.active||this.running||this.paused;this.pump();return {queued,...(queued&&this.runner.guidanceReason?{reason:this.runner.guidanceReason(item.taskId)}:{})};
 ...
 async flushGuidance(){
  ...
  try{
   while(this.runner.active&&!this.paused){
    const active=this.runner.active;
    const item=this.items.find(i=>i.taskId===active.task?.id&&i.status==='waiting'&&i.options?.steer);
    if(!item)break;
    const task=this.store.get(item.taskId);
    if(task.goalLifecycle?.status==='paused'||task.state==='unknown'||active.cancelRequested)break;
