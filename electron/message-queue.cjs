...
class MessageQueue extends EventEmitter{
 ...
 async send(id){
  ...
  item.status='dispatching';delete item.error;this.save();
  try{
   if(!item.options?.planning&&!task.planReviewRequired&&await this.runner.steer(item.taskId,item.text,item.images)){
   ...
   const queued=!!this.runner.active||this.running||this.paused;this.pump();return {queued};
