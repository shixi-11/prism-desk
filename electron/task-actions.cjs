...
function manageTask(store,runner,queue,id,action,value){
 ...
 case 'project':
  ...
  {const cwd=fs.realpathSync.native(value);if(task.cwd!==cwd){task.previousWorkspaces=[...new Set([...(task.previousWorkspaces||[]),task.cwd])];task.cwd=cwd;task.workspaceKind='project';task.sessions={};}task.projectGroup=true;task.section='';}break;
