function taskMenuTemplate(task,tasks,tr,choose,busy=false,savedProjects=[]){
 ...
 item('分享',{action:'share'}),{label:tr('复制'),submenu:[item('复制工作目录',{action:'copy-path'},{accelerator:'Ctrl+Shift+C'}),item('复制深度链接',{action:'copy-link'},{accelerator:'Alt+Ctrl+L'}),item('复制为 Markdown',{action:'copy-conversation'})]},divider,
