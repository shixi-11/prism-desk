...
class RunnerPool extends EventEmitter{
 ...
 blocker(task){return this.activeRuns().find(a=>a.task.id===task.id);}
 canRun(task){return !this.blocker(task);}
 assertIdle(task){this.forTask(task.id).assertIdle(task);if(!this.canRun(task))throw Error('当前会话仍在执行，请等待完成。');}
 ...
 guidanceReason(id){return this.forTask(id).guidanceReason(id);}
