...
**Solo para uso no comercial.** El código fuente se ofrece bajo la [licencia no comercial de Prism Desk](LICENSE). Se prohíbe el uso comercial sin autorización previa y por escrito de los titulares de los derechos.
...
*Capturas de la aplicación de escritorio real con conversaciones y nombres de cuenta de ejemplo. No se muestran datos privados de cuentas.*

### ¿Por qué Prism?

- **Cambia de cuenta y continúa.** La solicitud original, la conversación, las notas de progreso y los resultados de herramientas registrados acompañan a la misma tarea. Los archivos del proyecto permanecen en su sitio.
- **Avanza en varias partes de un proyecto.** Separa implementación, revisión e investigación en conversaciones que pueden ejecutarse a la vez, incluso en la misma carpeta.
- **Añade instrucciones sobre la marcha.** Envía mensajes mientras Codex, Claude o Grok trabajan. Elige instrucciones en directo o una cola, y controla las paradas y aprobaciones por conversación.

### Qué puedes hacer

| Función | En la práctica |
| --- | --- |
| Conversaciones en paralelo | Ejecuta varias conversaciones a la vez, incluso en carpetas iguales o anidadas. Detén o guía cada conversación por separado. |
| Instrucciones durante la ejecución | Envía requisitos a la conversación nativa activa de Codex, Claude o Grok. El CLI puede procesarlos tras la generación o el paso de herramienta actual. |
...
| Modo Fast de Codex | Acelera las respuestas de los modelos compatibles con un mayor consumo de créditos. Está desactivado por defecto, se guarda por cuenta dentro de cada tarea y se aplica a la siguiente ejecución sin cambiar el nivel de razonamiento. |
...
Estos comandos instalan la versión estable **v0.1.30**. Consulta la [última versión publicada](https://github.com/shixi-11/prism-desk/releases/latest) para conocer su número y leer las notas de cambios bilingües.
...
5. Cambia el nombre desde el menú contextual de la tarea, con doble clic o con **F2**. Pulsa **Enter** para guardar o **Esc** para cancelar.
...
Arrastra una conversación inactiva al nombre de otro proyecto para cambiar su carpeta de trabajo y conservar el historial y el borrador. Los archivos existentes permanecen en su ubicación original y las demás conversaciones no se combinan.
...
El mismo espacio de trabajo con temas claro y oscuro. Haz clic en una imagen para verla a tamaño completo.

| Tema claro · Inglés | Tema oscuro · Inglés |
| --- | --- |
| [![Tema claro · Inglés](output/20260912_prism-en-light.png)](output/20260912_prism-en-light.png) | [![Tema oscuro · Inglés](output/20260912_prism-en-dark.png)](output/20260912_prism-en-dark.png) |

<details>
<summary>Interfaz en chino</summary>

[![Interfaz en chino](output/20260912_prism-zh-dark.png)](output/20260912_prism-zh-dark.png)
...
En **Capacidades compartidas**, elige **Comprobar capacidades compartidas → Sincronizar fuentes locales** para comparar archivos y rutas con la última sincronización. Las habilidades del sistema y las carpetas añadidas entran en el índice después de sincronizar. **Conectar plugins instalados** registra en lote los plugins que informa Codex local y conecta sus referencias legibles. También activa la sincronización automática, que puedes desactivar en **Fuentes y conexiones**. Se comprueba al iniciar, al volver a la ventana y cada cinco minutos. Los plugins desactivados, desinstalados o excluidos manualmente no se vuelven a añadir automáticamente. Los originales permanecen en su sitio; el siguiente turno recibe el índice compartido.

La lista utiliza las versiones realmente instaladas, sin sustituirlas por una caché más reciente. Los plugins solo en caché se pueden mostrar aparte y no se importan en lote. Si falla la comprobación, se conserva la última lista y se detiene la importación automática. Registrar plugins o leer sus referencias no confirma acceso a herramientas ni autorización; el estado de conexión se muestra por separado. Codex MCP sigue necesitando configuración y verificación en la CLI independiente de cada cuenta. Los canales actuales de Claude, Grok y Gemini no habilitan MCP externo. Las herramientas ligadas a la aplicación de escritorio original se indican como no compatibles. La sincronización no copia credenciales ni claves API, ni activa permisos de herramientas.
...
**Solo para uso no comercial.** El código fuente se ofrece bajo la [licencia no comercial de Prism Desk](LICENSE). Se prohíbe el uso comercial sin autorización previa y por escrito de los titulares de los derechos. Prism Desk es un proyecto independiente. Los nombres y las marcas de los proveedores pertenecen a sus respectivos propietarios.
