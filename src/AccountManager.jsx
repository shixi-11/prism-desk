...
import {Plus,RefreshCw,ExternalLink,FolderOpen,Check,Settings2} from 'lucide-react';
...
function LoginStatus({state}){
 ...
 const labels={starting:'正在启动登录…',waiting:'请在浏览器完成授权',verifying:'正在检查账号…',connected:'账号已连接',cancelled:'已取消登录',error:'登录未完成'};
 return <div className={`connection-status ${state.phase}`} role="status">{state.phase==='connected'?<Check size={17}/>:active(state)?<RefreshCw size={17} className="spinning"/>:null}<span>{tr(labels[state.phase])}{state.message&&<small>{tr(state.message)}</small>}</span></div>;
}
  ...
  {editor?<section className="account-setup">
   ...
   {step==='form'?<form onSubmit={save}>
    ...
    <div className="connection-buttons">
      <button className="primary" disabled={locked||!current?.cliAvailable} onClick={()=>login(current.id)}><ExternalLink size={17}/>{tr('浏览器登录')}</button>
      ...
      {state?.hasUrl&&<button onClick={()=>action(()=>api.openAccountLogin(current.id))}>{tr('重新打开登录页')}</button>}
