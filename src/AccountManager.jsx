...
const focusWithoutScroll=button=>button?.focus({preventScroll:true});
...
function AccountEmail({email}){const [visible,setVisible]=useState(false);useEffect(()=>setVisible(false),[email]);return <small className="account-email"><span dir="ltr">{email?(visible?email:'••••••@••••••'):tr('邮箱暂未返回')}</span><button type="button" disabled={!email} aria-label={tr(visible?'隐藏邮箱':'显示邮箱')} aria-pressed={visible} onClick={()=>setVisible(v=>!v)}>{visible?<EyeOff size={15}/>:<Eye size={15}/>}</button></small>;}
function AccountName({profile,onSaved}){
 const [editing,setEditing]=useState(false),[name,setName]=useState(profile.name),[busy,setBusy]=useState(false),[error,setError]=useState('');
 useEffect(()=>{if(!editing)setName(profile.name);},[profile.name,editing]);
 async function save(e){e.preventDefault();setBusy(true);setError('');try{await api.renameAccount(profile.id,name);setEditing(false);await onSaved();}catch(e){setError(errorText(e));}finally{setBusy(false);}}
 return <div className="account-name">{editing?<form onSubmit={save}><label>{tr('备注名：')}<input autoFocus aria-label={tr('备注名')} value={name} maxLength={60} disabled={busy} onChange={e=>setName(e.target.value)} onKeyDown={e=>{if(e.key==='Escape'&&!busy){e.preventDefault();setEditing(false);}}}/></label><button disabled={busy||!name.trim()} aria-label={tr('保存备注名')} title={tr('保存备注名')}><Check size={16}/></button><button type="button" disabled={busy} onClick={()=>setEditing(false)} aria-label={tr('取消')}><X size={16}/></button></form>:<><span>{tr('备注名：')}{tr(profile.name)}</span><button aria-label={tr('修改备注名')+' '+profile.name} title={tr('修改备注名')} onClick={()=>{setName(profile.name);setError('');setEditing(true);}}><Pencil size={15}/></button></>}{error&&<small role="alert">{error}</small>}</div>;
}
 ...
 return <div className="account-manager">
  ...
  {editor?<section className="account-setup">
   ...
   {step==='form'?<form onSubmit={save}>
    ...
    <label>{tr('备注名')}<input dir="auto" autoFocus required maxLength={60} value={editor.name} onChange={e=>setEditor({...editor,name:e.target.value})} disabled={locked}/></label>
    ...
    {current?.connectionState==='connected'&&!active(state)&&<p className="connection-ready"><Check size={17}/>{tr(current.disabled?'登录已确认，账号当前已停用。':'此账号已加入执行账号列表。')}{current.email&&<AccountEmail email={current.email}/>}</p>}
