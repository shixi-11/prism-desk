...
export default function ProjectAccess({task,tasks,savedProjects=[],projectNames,onSave}){
 const [busy,setBusy]=useState(false),[error,setError]=useState('');
 const roots=task.linkedProjects||[],disabled=busy||['running','stopping','unknown'].includes(task.state);
 const choices=[...new Map([...savedProjects.map(p=>p.cwd),...tasks.flatMap(t=>[t.cwd,...(t.projectHistory||[])])].filter(dir=>dir&&key(dir)!==key(task.cwd)).map(dir=>[key(dir),dir])).values()].filter(dir=>!roots.some(root=>key(root)===key(dir)));
