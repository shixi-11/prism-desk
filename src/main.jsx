...
function TaskEntry({item,selected,onSelect,onRename,onAction}){
  ...
  useEffect(()=>{if(!selected||editing)return;const handler=e=>{const k=e.key.toLowerCase();const action=e.ctrlKey&&e.shiftKey&&!e.altKey&&k==='c'?'copy-path':e.ctrlKey&&e.altKey&&!e.shiftKey&&k==='l'?'copy-link':e.ctrlKey&&e.altKey&&k==='r'?'rename':e.ctrlKey&&e.altKey&&k==='p'?'pin':e.ctrlKey&&e.shiftKey&&k==='u'?'unread':e.ctrlKey&&e.shiftKey&&k==='a'?'archive':null;if(action&&!e.defaultPrevented){e.preventDefault();act({action});}};window.addEventListener('keydown',handler);return()=>window.removeEventListener('keydown',handler);},[selected,editing,item.id,onAction]);
    ...
    <aside className="inspector">
      ...
      <section>
        ...
        {selected?.provider==='Codex'&&<div className="reset-credit">
          ...
          <button className="quiet" disabled={resetBusy||stale||!(quota?.resetCredits?.availableCount>0)} onClick={async()=>{const id=target;setResetBusy(true);try{const result=await api.resetCredit(id);setResetMessage(({reset:tr("已使用一张重置卡"),alreadyRedeemed:tr("此前请求已完成，未重复消耗"),noCredit:tr("此账号没有可用重置卡"),nothingToReset:tr("当前额度无需重置，未消耗卡"),cancelled:tr("已取消")})[result.outcome]||tr("请核对重置结果"));if(result.outcome!=='cancelled')await onRefresh(id);}catch(e){setResetMessage(e.message+tr("；请先刷新核对结果，再次请求会复用未完成的操作编号。"));}finally{setResetBusy(false);}}}>{resetBusy?tr("正在处理…"):tr("使用重置卡")}</button>
