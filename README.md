...
Add each account to `.local/config.json` with its executable and the login directory used during sign-in. Restart Prism after changing this configuration, select the account and refresh its status. Missing login or a failed check must be resolved before that account can run a task. Claude accounts that are signed out show **Sign in again**, which opens the official subscription authorization page. Optional `email` and `subscriptionType` fields in each local profile identify the intended account and prevent an accidental login to a different subscription.
...
Claude 入口未登录时，账号页提供**重新登录**，打开官方订阅授权页。可在本地账号配置中填写 mail 和 subscriptionType，用于显示并核对对应邮箱和订阅类型。Gemini 暂不列入默认账号。
