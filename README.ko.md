...
다음 명령은 안정 버전 **v0.1.8**을 설치합니다. [최신 릴리스](https://github.com/shixi-11/prism-desk/releases/latest)에서 버전과 두 언어로 제공되는 변경 사항을 확인하세요.
...
Prism은 API 키 과금으로 전환하거나 크레딧을 구매하지 않습니다. Claude와 Grok을 실행하려면 추가 과금이 비활성화되어 있음을 확인해야 합니다. 기존 Codex 초기화 크레딧을 사용하려면 선택한 계정에 대해 별도로 확인해야 합니다. 작업 실행 중에도 초기화 크레딧을 사용할 수 있으며 작업은 계속 실행됩니다. 사용자 확인과 중복 요청 방지는 그대로 적용됩니다. 이미 종료된 턴은 자동으로 다시 시작되지 않습니다.
